...
const Infographics = () => {
  return (
    <section id="info" className="py-24 bg-white/5 backdrop-blur-md border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 mb-16 border-l-8 border-brutalist-red pl-8">
          <h2 className="brutal-heading-lg">Election <br />By The <span className="bg-slate-800 text-white px-4">Numbers</span></h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border border-white/10">
          {data.map((item, i) => (
            <motion.div 
              ...
              className={`p-10 flex flex-col gap-6 border-white/10 ${i !== data.length - 1 ? 'lg:border-r-4' : ''} ${i < 2 ? 'border-b-4 md:border-b-4 lg:border-b-0' : 'md:border-b-4 lg:border-b-0'} ${i === 1 ? 'md:border-r-0 lg:border-r-4' : ''}`}
            >
              <div className={`${item.color} ${item.color === 'bg-slate-800' ? 'text-white' : 'text-slate-200'} p-4 border border-white/10 w-fit shadow-premium`}>
                {item.icon}
              </div>
              <div>
        ...
        <div className="mt-16 bg-indigo-600 p-8 border border-white/10 shadow-premium text-white flex flex-col md:flex-row items-center justify-between gap-8">
