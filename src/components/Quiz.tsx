...
const Quiz = () => {
  ...
  return (
    <section id="quiz" className="py-24 bg-slate-800 text-white border-b border-white/10">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 mb-12 text-center items-center">
          <div className="bg-pink-600 px-4 py-2 border border-white inline-block shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]">
        ...
        <div className="brutal-card bg-white/5 backdrop-blur-md text-slate-200 p-8 md:p-12">
          {!showResult ? (
            <div className="space-y-8">
              <div className="flex justify-between items-center border-b-4 border-white/10 pb-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                {questions[currentQuestion].options.map((option, index) => (
                  <button
                    ...
                    disabled={isAnswered}
                    className={`
                      p-6 text-left text-xl font-bold border border-white/10 transition-all
                      ${!isAnswered ? 'hover:bg-indigo-600 hover:text-white hover:translate-x-1' : ''}
                      ${isAnswered && index === questions[currentQuestion].answer ? 'bg-cyan-500' : ''}
                      ${isAnswered && selectedOption === index && index !== questions[currentQuestion].answer ? 'bg-pink-600 text-white' : ''}
                      ${selectedOption === index ? 'shadow-none translate-x-1' : 'shadow-premium'}
                    `}
                  >
              ...
              <AnimatePresence>
                {isAnswered && (
                  <motion.div
                    ...
                    className="bg-gray-100 p-6 border border-white/10 border-dashed"
                  >
            ...
            <div className="text-center space-y-8 py-8">
              <div className="inline-block p-8 bg-cyan-500 border border-white/10 shadow-premium transform -rotate-2">
